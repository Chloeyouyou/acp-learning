...
const currentIndex = computed(() => Math.max(0, stages.indexOf(props.current)))
</script>

<template>
  <ol class="stage-track" aria-label="本关学习阶段">
    <li v-for="(stage, index) in stages" :key="stage"
      ...
      <span class="name">{{ stage.slice(1) }}</span>
    </li>
  </ol>
</template>

<style scoped>
.stage-track { list-style: none; display: grid; grid-template-columns: repeat(6, 1fr); gap: 4px; margin: 12px 0 14px; padding: 0; }
...
@media (max-width: 720px) {
  .stage-track { grid-template-columns: repeat(3, 1fr); row-gap: 8px; }
  .stage-track li:nth-child(3)::after { display: none; }
}
</style>
